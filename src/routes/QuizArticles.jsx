...
const QuizArticles = () => {
  ...
  const searchArticles = useCallback(async () => {
    try {
      ...
      setArticles(response.data.articles.filter((e, index, array) => {
  ...
  return (
    ...
    >
      <div
        className={'p-16 w-1/2 h-full flex flex-col items-start'}
      >
        <FormLabel
          width={'100%'}
        >
        ...
        {articles.slice(0, 10).map((article, index) => (
          <button
            className={cx(
              'first:mt-10 mt-3 w-full text-left hover:opacity-75 active:opacity-50',
              'py-3 px-4 rounded-lg duration-100 text-white',
              selectedArticles.find(e => e.url === article.url) ? 'bg-teal-800' : 'bg-teal-600'
            )}
      ...
      <div
        className={'p-16 w-1/2 h-full flex flex-col'}
      >
        <div
        ...
        <div
          className={'grid grid-cols-4 grid-rows-4 gap-6 mt-4'}
        >
